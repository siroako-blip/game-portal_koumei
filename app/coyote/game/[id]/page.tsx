...
import type { CoyoteGameState, CoyoteCard } from "@/app/coyote/logic";
import {
  bid,
  callCoyote,
  startNextRound,
  createInitialCoyoteState,
  restartGame,
} from "@/app/coyote/logic";
import { useCoyoteRealtime } from "@/app/coyote/useRealtime";
import { usePresenceMany } from "@/lib/usePresence";
import { PresenceDot } from "@/components/PresenceDot";
import {
  startCoyoteGame,
  updateCoyoteGameState,
} from "@/lib/gameDb";
...
function cardLabel(c: CoyoteCard): string {
...
function GameContent() {
  ...
  const { gameData, loading, error } = useCoyoteRealtime(gameId);
  ...
  const state: CoyoteGameState | null = gameData?.game_state ?? null;

  const handleStartGame = useCallback(async () => {
    if (!gameId || !isHost || playerIds.length < MIN_PLAYERS || playerIds.length > MAX_PLAYERS) return;
    setIsSubmitting(true);
    try {
      const initialState = createInitialCoyoteState(playerIds.length);
      await startCoyoteGame(gameId, initialState);
    } finally {
  ...
  const handleBid = useCallback(
    async (value: number) => {
      ...
      setBidInput("");
      try {
        await updateCoyoteGameState(gameId, next);
      } finally {
        setIsSubmitting(false);
      }
    },
  ...
  const handleCoyote = useCallback(async () => {
    if (isSpectator || myIndex < 0 || !gameId || !state) return;
    const next = callCoyote(state, myIndex);
    if (!next) return;
    setIsSubmitting(true);
    try {
      await updateCoyoteGameState(gameId, next);
  ...
  if (error || !gameData) {
    return (
      <div className="min-h-screen flex flex-col p-4 gap-4 items-center justify-center bg-gradient-to-b from-purple-950 to-indigo-950 text-purple-100">
        ...
        <Link href="/coyote" className="text-fuchsia-400 underline font-medium">
          ロビーに戻る
        </Link>
      </div>
    );
  ...
  const canCoyote = isMyTurn && !isSpectator && !isSubmitting && state.currentBid >= 0;
...
export default function CoyoteGamePage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex flex-col p-4 gap-4 items-center justify-center bg-gradient-to-b from-purple-950 to-indigo-950 text-purple-100">
