/**
 * Coyote（コヨーテ）対戦ゲームロジック
 * 自分のカードだけ見えず、合計値を推理してビッド or Coyote! でチャレンジ。
 */

export type CoyoteCard = number | "x2" | "MAX=0" | "?";

/** デッキ: 10刻みの大きな数字＋特殊。数字は互いに近すぎずハッタリしやすい構成 */
const NUMBERS_10_80 = [10, 20, 30, 40, 50, 60, 70, 80];
export const FULL_DECK: CoyoteCard[] = [
...
export function isNumericCard(c: CoyoteCard): c is number {
...
export function calculateTotal(
  hands: CoyoteCard[][],
  deck: CoyoteCard[]
): { total: number; usedDeck: CoyoteCard[] } {
  const usedDeck = [...deck];
  const allCards: CoyoteCard[] = hands.flat();
...
export interface CoyoteGameState {
  ...
  revealedHands?: CoyoteCard[][];
  /** 再戦に同意したプレイヤーの pid 配列（全員同意で restartGame） */
  rematchVotes?: string[];
}
...
export function createInitialCoyoteState(playerCount: number): CoyoteGameState {
...
function nextPlayerIndex(state: CoyoteGameState): number {
...
/** Coyote! チャレンジ。宣言者 vs 前の宣言者のどちらが負けかを判定 */
export function callCoyote(state: CoyoteGameState, playerIndex: number): CoyoteGameState | null {
  if (state.phase !== "bidding") return null;
  if (state.currentPlayerIndex !== playerIndex) return null;
  if (state.lives[playerIndex] <= 0) return null;
...
export function startNextRound(state: CoyoteGameState): CoyoteGameState | null {
 ...
 */
export function restartGame(state: CoyoteGameState): CoyoteGameState | null {
  if (state.phase !== "gameover") return null;
  return createInitialCoyoteState(state.hands.length);
}
