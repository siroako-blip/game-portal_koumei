...
export type ItoDifficulty = "EASY" | "NORMAL" | "HARD" | "MIXED" | "GRADUAL";
...
export function getNewTheme(difficulty: ItoDifficulty, level: number): string {
...
export function createInitialItoState(
  playerCount: number,
  difficulty: ItoDifficulty = "MIXED"
): ItoGameState {
...
export function changeTheme(state: ItoGameState): ItoGameState | null {
 ...
 */
export function restartGame(state: ItoGameState): ItoGameState {
  const playerCount = state.players.length;
...
export function resetGame(state: ItoGameState): ItoGameState {
