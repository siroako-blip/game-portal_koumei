...
export interface WordWolfMessage {
  author: string;
  text: string;
  timestamp: number;
}

export type WordWolfPhase = "discussion" | "voting" | "result";

export interface WordWolfGameState {
  phase: WordWolfPhase;
  /** 多数派の単語（市民） */
  majorityWord: string;
  /** 少数派の単語（ウルフ） */
...
export function createInitialWordWolfState(playerCount: number): WordWolfGameState {
...
export function getPlayerWord(state: WordWolfGameState, playerIndex: number): string {
  return state.assignments[playerIndex] === 0 ? state.majorityWord : state.minorityWord;
}

/** 議論フェーズを終了し、投票フェーズへ */
export function endDiscussion(state: WordWolfGameState): WordWolfGameState | null {
...
export function addMessage(
  ...
  author: string,
  text: string
): WordWolfGameState | null {
  if (state.phase !== "discussion") return null;
  const trimmed = text.trim();
  if (!trimmed) return null;
  const message: WordWolfMessage = {
    author,
    text: trimmed,
    timestamp: Date.now(),
  };
...
/** 全員投票済みか */
function allVoted(state: WordWolfGameState): boolean {
  return state.votes.every((v) => v >= 0);
}

/** 最多得票者の index を返す（同数の場合は先にその得票に達した人＝最初の同点） */
function getMostVotedIndex(state: WordWolfGameState): number {
...
export function finishVoting(state: WordWolfGameState): WordWolfGameState | null {
  if (state.phase !== "voting") return null;
...
export function getRemainingDiscussionSeconds(state: WordWolfGameState): number {
 ...
 */
export function restartWordWolfGame(
  state: WordWolfGameState
): WordWolfGameState | null {
  if (state.phase !== "result") return null;
  return createInitialWordWolfState(state.assignments.length);
}
