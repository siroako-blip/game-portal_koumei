...
- **節目となる開発を終えたら、ユーザーに頼まれなくても必ず `DEVLOG.md` のいちばん上に追記する**（逆時系列）。これは任意ではなく必須の手順。フォーマットは `DEVLOG.md` 冒頭参照。「何を・なぜ」を1〜数行で。細かな修正は不要、方針転換・機能追加・大きなリファクタなど節目のみ。
  - 「節目」の目安: 新機能の追加、方針転換、大きなリファクタ、本番デプロイなど。複数の小さなコミットがまとまって1つの節目になることもある。
  - 追記は原則として、その節目のコミットと同じタイミング（同一コミットか直後のコミット）で行う。
